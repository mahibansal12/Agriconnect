  ...
  return (
    <div style={{
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <p style={{ margin:"0 0 20px", color:"#a7f3d0", fontSize:"14px" }}>📍 {location}</p>

        <div style={{ display:"flex", alignItems:"center", gap:"20px", marginBottom:"24px" }}>
          <span style={{ fontSize:"70px", lineHeight:1 }}>{current.icon}</span>
          <div>
            <p style={{ margin:0, fontSize:"52px", fontWeight:900, lineHeight:1 }}>{current.temp}°C</p>
            <p style={{ margin:"4px 0 0", color:"#a7f3d0", fontSize:"15px" }}>{current.condition}</p>
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"12px", borderTop:"1px solid rgba(134,239,172,0.25)", paddingTop:"20px" }}>
          {[
            ...
            { label:"Rain",     val:`${current.rainChance}%`, icon:"🌧️" },
          ].map(item => (
